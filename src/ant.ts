...
export class Ant {
  x: number;
  y: number;
  static sizeX:number  = 25
  static sizeY: number = 40
  vec: Vector;
  size: number;
  strength: number;
  name: string = "Bob";
  ...
  static image = document.getElementById("ant")! as HTMLCanvasElement;
  ...
  followStrongestSmell() {
    ...
    let vecSum = new Vector(
      this.vec.x * 0.0001 + (Math.random() - 0.5) * 0.00005,
      this.vec.y * 0.0001 + (Math.random() - 0.5) * 0.00005
    );
  ...
  wallCheck() {
    ...
    if (
      simu.terrain.grid[xx][yy] == Terrain.block ||
  ...
  foodCheck() {
    ...
    let yy = Math.floor(this.y);
    if (
      simu.terrain.grid[xx][yy] == Terrain.food ||
  ...
  colonyCheck() {
    if (
      Math.abs(this.x - this.owner.x) < this.owner.sizes / 2 &&
